            ...
            <form class="cmxform" id="detailsform" method="get" action="{{route('invoice.new')}}" enctype="multipart/form-data">
               <fieldset>
                  ...
                  <div class="form-group">
                     ...
                     <select class="js-example-basic-single w-100" name="customer_id" required="" id="customer_id">
                        <option value="">--Select Customer --</option>
                        @foreach($customers as $customer)
                        <option value="{{$customer->id}}"  data-gstin="{{ $customer->gstin }}"  @php if($customer->id == $id){ echo "selected='selected'"; }  @endphp>{{$customer->fullname}} - {{$customer->email}}</option>
                        @endforeach
                     </select>
                  </div>
                  {{-- Row Start Here --}}
                  <div class="row">
                     ...
                     <div class="col-md-4">
                        <div class="form-group">
                           <label for="class">Select Madni State
                           <small class="text-danger">*</small>
                           </label>
                           <select class="js-example-basic-single w-100" name="company_state" id="company_state" required="">
                              <option value="">--Select Company State --</option>
                              @foreach($states as $state)
                                <option value="{{ $state->id }}" data-gst_code="{{ $state->gst_code }}">{{ $state->name }}</option>
                              @endforeach 
                           </select>
                        </div>
                     </div>
                     <div class="col-md-4">
                        <div class="form-group">
                           <label for="class">Select Customer State
                           <small class="text-danger">*</small>
                           </label>
                           <select class="js-example-basic-single w-100" name="customer_state" id="customer_state" required="">
...
<script>
   ...
   $("#customer_id").on('change',function(){
      ...
      if(gstin) {
         var custom_gst_code = gstin.substring(0, 2)
         console.log('custom_gst_code',custom_gst_code);
         $("#customer_state option[data-gst_code='" + custom_gst_code + "']").prop("selected", true).trigger('change');
      } else {
         $("#customer_state option").prop("selected", false).trigger('change')
      }
